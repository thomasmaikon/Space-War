...
void Player::Update()
{
    ...
    else
    {
        ...
        {
            arma->DisparoPosicao(firingAngle, x, y, nullptr);
    ...
    float spaceBetween = Point::Distance(Point(x, y), Point(BasicAI::blackHole->X(), BasicAI::blackHole->Y()));
    float gravityForce = BasicAI::blackHole->Forca() * (300 - (spaceBetween >= 300 ? 300 : spaceBetween));
    Move(Vector(angle, gravityForce*gameTime));
